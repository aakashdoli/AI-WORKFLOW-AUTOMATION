...
from app.models.schema import Workflow, PipelineExecution, ProcessedResult, ExecutionStatus

def seed_data():
    print("🚀 Initializing AI Ops Demo Data Seeding...")
    init_db()
    db = DatabaseService()
    ...
    for i in range(150):
        ...
        # Add a sample result for each execution
        if status == ExecutionStatus.COMPLETED:
            ...
            result = ProcessedResult(
                ...
                input_data=json.dumps({"id": f"REC-{i}", "source": "Internal Batch"}),
                output_data=f"AI processing finalized for {w.name}. Analysis indicates a {category} event with {priority} priority.",
                category=category,
                priority=priority,
    ...
    db.db.commit()
    print("✅ Demo Data Seeding Complete. Platform is now alive!")
    db.close()
